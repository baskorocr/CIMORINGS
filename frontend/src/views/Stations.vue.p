    ...
    <div class="stations-container">
          ...
          <div class="table-responsive">
            ...
            <el-table-column label="Actions" width="180" fixed="right">
              <template #default="scope">
                ...
                <el-button size="small" text @click.stop="getDiagnostics(scope.row)">
            ...
            <el-col :span="24">
              <el-card>
                ...
                <div style="text-align: center; padding: 20px; color: #909399;">
                  Click "Download Messages" to get OCPP message logs for this station
                </div>
              </el-card>
            </el-col>
          </el-row>
      ...
      >
        ...
        <div class="diagnostics-list">
          <div v-for="item in filteredDiagnosticsData" :key="item.id" class="diagnostic-item">
            <div class="diagnostic-header">
              <span class="diagnostic-id">ID: {{ item.id }}</span>
              <el-tag :type="getStatusType(item.status)" size="small">{{ item.status }}</el-tag>
            </div>
            <div class="diagnostic-info">
              <div class="info-row">
                <span class="label">Location:</span>
                <span class="value">{{ maskFtpCredentials(item.location) }}</span>
              </div>
              <div class="info-row">
            ...
            <div class="diagnostic-actions" v-if="item.status === 'Uploaded'">
              <el-button type="primary" size="small" @click="downloadDiagnosticFile(item)">
                <el-icon><Download /></el-icon>
                Download ZIP
              </el-button>
            </div>
        ...
        <div v-if="diagnosticsData.length === 0" class="empty-state">
    ...
    async downloadOCPPMessages(station) {
      try {
        ...
        if (response.ok) {
          ...
          a.style.display = 'none'
          a.href = url
          a.download = `ocpp_messages_${station.charge_point_id}_${new Date().toISOString().split('T')[0]}.json`
          document.body.appendChild(a)
          a.click()
          window.URL.revokeObjectURL(url)
          document.body.removeChild(a)
